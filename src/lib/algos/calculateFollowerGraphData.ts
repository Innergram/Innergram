...
export default function calculateFollowerGraphData(
  followers: Follower[] | undefined,
) {
  const sortedFollowers = followers?.sort(
    (a, b) => a.followed_at - b.followed_at,
  );

  const result: { x: string; y: number; followersAtPresent: number }[] = [];
  sortedFollowers?.forEach((follower, idx) => {
    ...
    const followersInWindow = sortedFollowers.filter(
      (f) =>
        f.followed_at >= windowStart.unix() &&
        f.followed_at <= currentTime.unix(),
    );

    const uniqueFollowers = [
      ...new Set(followersInWindow.map((f) => f.username)),
    ];
    const growthDifference =
      idx > 0 ? uniqueFollowers.length - result[idx - 1].y : 0;

    result.push({
      x: currentTime.format("MMMM YYYY"),
      y: uniqueFollowers.length,
      followersAtPresent: idx + growthDifference,
    });
  });
